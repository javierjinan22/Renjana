...
public class DaftarController implements Initializable {
    ...
    LinkedList<Pengguna> dataRegistration = new LinkedList<>(); //inisialisasi LinkedList untuk  menyimpan data Pengguna yang dibutuhkan saat login

    XStream xstream = new XStream(new StaxDriver()); //Untuk mengubah data ke XML
    ...
    private void daftarButton(ActionEvent Event) throws IOException {
        ...
        boolean manager = false; //Akun ini bukan sebagai manager
        boolean admin = false; //Akun ini bukan sebagai admin

        boolean similarEmail = false; //variabel digunakan untuk mengecek apakah ada Email yang sama (telah digunakan)
